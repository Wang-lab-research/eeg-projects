...
def to_source(sub_id,data_path,epo_path,save_path_cont,
    ...
    # Make subpaths
    subpath_cont,subpath_zepo = make_sub_time_win_path(sub_id,save_path_cont,
                                                       save_path_zepo,
                                                       include_zepochs=True)
    
    # Quick check to see if subject already processed
    if len(os.listdir(subpath_cont))<len(roi_names) or len(os.listdir(subpath_zepo))<len(roi_names):
